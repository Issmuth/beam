...
function ChatContent() {
    ...
    const getErrorMessage = (error, statusCode) => {
        ...
        if (statusCode) {
            switch (statusCode) {
                case 400: return 'Invalid request. Please try rephrasing your message.';
                case 500: return 'Server configuration error. Please try again later.';
                case 502: return 'The AI service is temporarily unavailable. Please try again in a moment.';
                case 429: return 'Too many requests. Please wait a moment before trying again.';
                default: return statusCode >= 500 ? 'Server error. Please try again later.' : 'Something went wrong. Please try again.';
            }
        }
        if (!navigator.onLine) {
    ...
    const handleSendMessage = async (messageText, isRetry = false) => {
        ...
        if (!isRetry) {
            setMessages((prev) => [...prev, { role: 'user', content: trimmedMessage, timestamp: Date.now() }]);
        }
        ...
        try {
            const conversationHistory = messages.filter((msg) => !msg.isError).map((msg) => ({ role: msg.role, content: msg.content }));
            const response = await fetch('/api/generate', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ message: trimmedMessage, conversationHistory }),
            });
            ...
            setMessages((prev) => [...prev, { role: 'system', content: data.text, timestamp: Date.now() }]);
            ...
            setMessages((prev) => [...prev, { role: 'system', content: getErrorMessage(error, error.statusCode || statusCode), timestamp: Date.now(), isError: true }]);
        } finally {
            setIsLoading(false);
        }
    };

    const handleRetry = () => lastFailedMessage && handleSendMessage(lastFailedMessage, true);
    const handleSubmit = (value) => handleSendMessage(value);
        ...
        <main className="min-h-screen flex flex-col">
                ...
                <div className="max-w-3xl mx-auto">
                    ...
                    {messages.map((message, index) => (
                        <div key={`${message.timestamp}-${index}`}>
                            <ChatBubble role={message.role} content={message.content} timestamp={message.timestamp} isError={message.isError} />
                            {message.isError && lastFailedMessage && index === messages.length - 1 && !isLoading && (
                                <div className="flex justify-start mb-4 ml-2">
                                    <button onClick={handleRetry} className="flex items-center gap-2 px-4 py-2 text-sm text-white glass-card hover:bg-white/10 transition-all duration-200">
            ...
            {roadmapData && (
                <div className="px-4 md:px-6 py-3">
                    <div className="max-w-3xl mx-auto">
                        <button onClick={handleViewRoadmap} className="w-full py-3.5 bg-beam-gradient text-white font-medium rounded-2xl hover:shadow-glow transition-all duration-300 flex items-center justify-center gap-2">
            ...
            <div className="px-4 md:px-6 py-4 border-t border-white/5">
                <div className="max-w-3xl mx-auto">
                    <InputBar value={inputValue} onChange={setInputValue} onSubmit={handleSubmit} placeholder="Type your message..." disabled={isLoading} />
                </div>
            </div>
        </main>
    );
