        ...
        <main className="min-h-screen flex flex-col">
                ...
                <div className="mx-4 sm:mx-6 lg:mx-8 mt-4 animate-slide-up">
                    <div className="max-w-2xl mx-auto">
                        <div className="glass-card border-amber-500/30 px-4 py-3 flex items-center justify-between">
                            ...
                            <button onClick={() => setErrorMessage(null)} className="p-2 rounded-lg text-white/70 hover:text-white hover:bg-white/10 transition-all" aria-label="Dismiss">
            ...
            <div className="w-full px-4 sm:px-6 lg:px-8 pb-8 sm:pb-12">
                <div className="max-w-2xl mx-auto animate-slide-up">
                    <InputBar value={goal} onChange={setGoal} onSubmit={handleSubmit} placeholder="e.g., Learn React in 3 months..." />
                    <div className="flex flex-wrap justify-center gap-2 mt-4">
                        {['Learn Python', 'Master React', 'Data Science', 'Machine Learning'].map((suggestion) => (
                            <button key={suggestion} onClick={() => setGoal(suggestion)} className="px-4 py-2 text-sm rounded-full bg-white/5 text-beam-text-secondary hover:bg-white/10 hover:text-white transition-all duration-200 border border-white/10">
                                {suggestion}
                            </button>
                        ))}
                    </div>
